...
class Vue():
    def __init__(self, parent, ip, nom):
        ...
        self.button = Button(self.root, text="X", command=self.root.destroy, font='arial 20', relief=FLAT, bg='#1c4873',
                             foreground="white")
    ...
    def creercadresplash(self, ip, nom):
        ...
        self.titre = Label(self.cadresplash, text="Bienvenue dans la galaxie orion voyageur!", bg='#15243d',
                           font='arial 20', foreground="white")
        self.titre.pack(pady=(100, 20), padx=100);

        soustitre = Label(self.cadresplash, text="Pour des fin de securite veuillez vous identifier", bg='#15243d',
                          font='arial 16', foreground="white")
        soustitre.pack(pady=10, padx=10);

        self.nomsplash = Entry(self.cadresplash, bg='#A3C5D8', relief=FLAT, foreground="white", font='arial 14',
                               highlightthickness=2, highlightcolor='#849fae')
        self.nomsplash.insert(0, nom)
        self.nomsplash.pack(pady=10)

        self.ipsplash = Entry(self.cadresplash, bg='#A3C5D8', relief=FLAT, foreground="white", font='arial 14',
                              highlightthickness=2, highlightcolor='#849fae')
        ...
        btnconnecterpartie = Button(self.cadresplash, text="Connecter partie", bg='#A3C5D8',
                                    command=self.connecterpartie, relief=FLAT, font='arial 12')
    ...
    def creercadrelobby(self):
        ...
        self.titre = Label(self.cadrelobby, text="Rebonjour Voyageur", bg='#15243d', font='arial 20',
                           foreground="white")
        ...
        btnlancerpartie = Button(self.cadrelobby, text="Lancer partie", command=self.lancerpartie, bg='#A3C5D8',
                                 relief=FLAT, font='arial 12')
    ...
    def creeraffichercadrepartie(self, mod):
        ...
        self.labidcolons = Label(self.boiteinfo, text="Colons disponibles: ", font=("Helvetica", 10), bg="#455571",
                                 fg="#fbbfda")
        self.labidcolons.pack(side=LEFT)
        self.labidcbcolons = Label(self.boiteinfo, text=joueur.totalcolons, font=("Helvetica", 10), bg="#455571",
                                   fg="#fbbfda")
        self.labidcbcolons.pack(side=RIGHT)

        self.boutoncolonsajout = Button(self.cadreinfo, text="[CREER COLONS]", font=("Helvetica", 8), bg="#455565",
                                        fg="#fbbfda", relief=RAISED, command=self.modifcolonsmodele(1))
        ...
        self.btncreervaisseau = Button(self.cadreinfo, text="Vaisseau", font=("Helvetica", 8), bg="#455565",
                                       fg="#fbbfda", relief=RAISED, command=self.creervaisseau)
    ...
    def afficherdecor(self, mod):
        ...
        print(len(mod.AI.planetecontrolees))
    ...
    def afficherplanetemereAI(self, evt):
        couleur = self.mod.Ai.couleur
        x = self.mod.AI.planetemere.x
        y = self.mod.AI.planetemere.y
        t = 10
        self.canevas.create_oval(x - t, y - t, x + t, y + t, dash=(3, 3), width=2, outline=couleur,
                                 tags=("planetemere", "marqueur"))
            ...
            for v in j.flotte:
                if nonControlee:
                    if abs(v.x - p.x) <= 3 and abs(v.y - p.y) <= 3:
                        ...
                        self.canevas.itemconfig(self.canevas.find_closest(v.x, v.y), fill=j.couleur, outline="black",
                                                tags=(p.proprietaire, "planete", str(p.id), "possession"))
                        nonControlee = False
